...
public class PagamentoService {
    ...
    private boolean simularProcessamentoPagamento() {
        // Simular processamento - em produção seria integração com gateway de pagamento
        try {
            Thread.sleep(1000); // Simular delay do processamento
            // Simular 95% de aprovação
    ...
    private void validarPagamento(Pagamento pagamento) throws ValidationException {
        ...
        // Nota: Removido validação de método de pagamento pois não existe no modelo
    }
    
    public List<Pagamento> listarPorAgendamento(int agendamentoId) {
        return dao.findWithQuery("FROM Pagamento p WHERE p.agendamentoId = ? ORDER BY p.dataPagamento DESC", agendamentoId);
    }
